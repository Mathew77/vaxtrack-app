...
const VaccineStockTable = () => {
  ...
  const { data, isLoading, error } = useFetchScsDashboard();
                ...
                <>
                  ...
                  >
                    ...
                    {vaccines.map(vaccine => {
                      ...
                      const maximum = (lgaData.sum_of_vaccine_at_lga_level as any)[fieldName] ?? 0;
                      const received = (lgaData.sum_of_vaccine_at_lga_level as any)[`${fieldName}_received`] ?? 0;
                      const physical = (lgaData.sum_of_vaccine_at_lga_level as any)[physicalFieldName] ?? 0;
                  ...
                  {/* Ward Rows */}
                  {expandedLGAs[lgaKey] && lgaData.wards.map((ward) => {
                    const wardKey = `${lgaData.state}-${lgaData.lga}-${ward.ward}`;
                    return (
                      <>
                        <TableRow
                          key={wardKey}
                        ...
                        >
                          ...
                          {vaccines.map(vaccine => {
                            const fieldName = vaccineFieldMap[vaccine];
                            const physicalFieldName = physicalStockFieldMap[vaccine];
                            const maximum = (ward.sum_of_vaccine_at_ward_level as any)[fieldName] ?? 0;
                            const received = (ward.sum_of_vaccine_at_ward_level as any)[`${fieldName}_received`] ?? 0;
                            const physical = (ward.sum_of_vaccine_at_ward_level as any)[physicalFieldName] ?? 0;
                            const percentage = maximum > 0 ? (((received + physical) / maximum) * 100).toFixed(1) : '0';
                            return (
                              <Tooltip
                                key={vaccine}
                        ...
                        {/* Facility Rows */}
                        {expandedWards[wardKey] && ward.facilities.map((facility) => (
                          <TableRow
                            key={facility.id}
                            sx={{
                              bgcolor: 'grey.100',
