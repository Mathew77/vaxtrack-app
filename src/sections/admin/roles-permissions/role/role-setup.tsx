...
import { RoleType } from 'src/hooks/apis/roles-permissions/roles-type';
      ...
      <Grid container spacing={2}>
        <Grid item xs={6}>
          ...
          <TextField
            ...
            variant="outlined"
            disabled={isView}
            helperText={errors?.name && <span style={{ color: '#DC143C', fontSize: '13px' }}>{errors?.name}</span>}
          />
        </Grid>

        <Grid item xs={6}>
          <Typography component="label" htmlFor="fk_org_unit_level_id">
            Organisation level unit
          </Typography>
          <Select
            fullWidth
