...
export const RotaVaccine = ({
  ...
  const calculateMismatchOutcome = (antigenStock: string, comparatorStock: string): string => {
    if (!antigenStock.trim() || !comparatorStock.trim()) {
      return '';
    }
    const antigenValue = parseFloat(antigenStock) || 0;
    const comparatorValue = parseFloat(comparatorStock) || 0;
    if (antigenValue === 0 && comparatorValue === 0) {
      return '';
    }
    if (comparatorValue === antigenValue) {
      return 'none';
    } else if (comparatorValue > antigenValue) {
      return 'excess';
    } else {
      return 'deficit';
    }
  ...
  const calculateMismatchAdjustedValue = (antigenStock: string, comparatorStock: string): string => {
    if (!antigenStock.trim() || !comparatorStock.trim()) {
      return '';
    }
    const antigenValue = parseFloat(antigenStock) || 0;
    ...
    const adjustedValue = antigenValue - comparatorValue;
  ...
  const handleInputChange = (field: keyof RotaVaccineData) => (
    ...
    setFormData((prev) => {
      ...
      if (field === 'physicalStock' || field === 'dropperPhysicalStock') {
        ...
        newFormData.dropperMismatchAdjustedValue = calculateMismatchAdjustedValue(antigenStock, dropperStock);
      }
