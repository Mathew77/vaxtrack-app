...
export const MeaslesVaccine = ({
  ...
  const calculateLcsTwoMlSyringeMismatch = (lcsSentSyringe: string, antigenStock: string): string => {
    if (!lcsSentSyringe.trim() || !antigenStock.trim()) {
      return '';
    }
    const syringeSent = parseFloat(lcsSentSyringe) || 0;
    const antigenAmount = parseFloat(antigenStock) || 0;
    const syringeBaseValue = antigenAmount / 20;
  ...
  const calculateSlwgTwoMlSyringeMismatch = (slwgSentSyringe: string, lcsSentSyringe: string): string => {
  ...
  const calculateScsTwoMlSyringeMismatch = (scsSentSyringe: string, slwgSentSyringe: string): string => {
  ...
  const calculateThreePlTwoMlSyringeMismatch = (threePlSentSyringe: string, scsSentSyringe: string): string => {
  ...
  const calculateEhfTwoMlSyringeMismatch = (ehfReceivedSyringe: string, threePlSentSyringe: string): string => {
  ...
  const handleInputChange = (field: keyof MeaslesVaccineData) => (
    ...
    setFormData((prev) => {
      ...
      if (field === 'lcsTwoMlSyringeQuantitySent') {
        newFormData.lcsTwoMlSyringeMismatchAdjusted = calculateLcsTwoMlSyringeMismatch(value, newFormData.physicalStock);
      }
      ...
      if (field === 'slwgTwoMlSyringeQuantitySent') {
        newFormData.slwgTwoMlSyringeMismatchAdjusted = calculateSlwgTwoMlSyringeMismatch(value, newFormData.lcsTwoMlSyringeQuantitySent || '');
      }
      ...
      if (field === 'scsTwoMlSyringeQuantitySent') {
        newFormData.scsTwoMlSyringeMismatchAdjusted = calculateScsTwoMlSyringeMismatch(value, newFormData.slwgTwoMlSyringeQuantitySent || '');
      }
      ...
      if (field === 'threePlTwoMlSyringeQuantitySent') {
        newFormData.threePlTwoMlSyringeMismatchAdjusted = calculateThreePlTwoMlSyringeMismatch(value, newFormData.scsTwoMlSyringeQuantitySent || '');
      }
      ...
      if (field === 'ehfTwoMlSyringeQuantityReceived') {
        newFormData.ehfTwoMlSyringeMismatchOutcome = calculateEhfTwoMlSyringeMismatch(value, newFormData.threePlTwoMlSyringeQuantitySent || '');
      }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <Box sx={sectionBorderStyle}>
        <Typography variant="subtitle1" sx={{ mb: 2 }}>Measles & Rubella Antigen</Typography>
      ...
      <Box sx={sectionBorderStyle}>
        <Typography variant="subtitle1" sx={{ mb: 2 }}>Measles & Rubella Diluent</Typography>
        <Grid container spacing={3}>
          <Grid item xs={6}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              <InputLabel>Physical Stock Balance</InputLabel>
